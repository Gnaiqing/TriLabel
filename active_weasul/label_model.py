...
class LabelModel:
    ...
    def fit(self,
        ...
        label_matrix = np.array(dataset_train.weak_labels)
        cliques = []  # assert conditional independence on LFs
        for i in range(dataset_train.n_lf):
            cliques.append([i])

        if y_valid is None:
            if dataset_valid is not None:
                y_valid = dataset_valid.labels
            else:
                # use the prediction result of majority vote to predict class balance
                mv = MajorityVoting()
                mv.fit(dataset_train)
                y_valid = mv.predict(dataset_train)

        y_valid = np.array(y_valid)
        class_balance = []
        for i in range(dataset_train.n_class):
            class_balance.append(float(np.mean(y_valid == i)))

        class_balance = np.array(class_balance)
        self._fit(label_matrix, cliques, class_balance, ground_truth_labels)
